          ...
          <div class="card bg-secondary shadow border-0">
            <div class="card-body px-lg-5 py-lg-4">
              <div class="text-center text-muted mb-4">
                <small>Ingresa tu correo electrónico, te enviaremos un codigo de recuperación.</small>
              </div>
              ...
              <form id="frm-forgot" role="form" method="post" action="<?= base_url().'login/sendCode' ?>">
                <div class="form-group">
                  <div class="input-group input-group-alternative mb-3">
                    <div class="input-group-prepend">
                      <span class="input-group-text"><i class="ni ni-email-83"></i></span>
                    </div>
                    <input id="forgotEmail" name="forgotEmail" class="form-control" placeholder="Correo electrónico" type="email" required>
                  </div>
                </div>
                <div class="text-center">
                  <button id="btn-sendcode" type="submit" class="btn btn-primary mt-1">Enviar código</button>
                </div>
              </form>
            </div>
          </div>
      ...
      const base_url = $('#base_url').val();
        ...
        $('#frm-forgot').bind("submit",function(){
          ...
          $.ajax({
            ...
            data:$(this).serialize(),
            beforeSend: function(){
              btnEnviar.text("Enviando còdigo...");
              // Para input de tipo button
              btnEnviar.attr("disabled","disabled");
              $("#forgotEmail").attr("disabled","disabled");
            },complete:function(data){
              btnEnviar.text("Enviar còdigo");
              btnEnviar.removeAttr("disabled");
              $("#forgotEmail").removeAttr('disabled');
              ...
              if(data == 'ok'){
                ...
                $("#resp_msg").hide()
                $("#success").show();
                $("#success").text('Envío exitoso, revisa tu cuenta para continuar con la recuperación de tu contraseña.');
              } else {
                $('#resp_reg').show();
                $("#resp_msg").text(data);
                $("#success").hide();
              ...
              $('#resp_reg').show();
              $("#success").hide();
              $("#resp_msg").text("Problemas al tratar de enviar el código.");
              btnEnviar.text("Enviar còdigo");
              btnEnviar.removeAttr("disabled");
            }
          });
